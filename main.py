...
def init():
	gmail=input(f'{C}[{Y}Gmail{C}]: ');password=getpass.getpass(prompt=f'{C}[{Y}password{C}]: ')
	login = {
	'log1':f'{gmail}',
	'log2':f'{password}',
	############
	...
	try:
		...
		print(f"{error} Compruebe si se ha activado la opción 'Aplicaciones menos seguras' o si ha introducido el correo electrónico / contraseña correctamente.\n{warning}: "+str(erro));time.sleep(5)
Sair = False
while(Sair == False):
		try:
